...
rm -f "$SOCK"
( cd "$HERE" && PORT=18902 PYLON_SOCKET="$SOCK" \
    QTAMP_BACKEND_URL="http://127.0.0.1:$BPORT" \
    PYLON_DISABLE_TELEMETRY=true \
    node .pylon/index.js >"$tmp/pout" 2>&1 ) &
PPID_PYLON=$!
for _ in $(seq 1 60); do
  curl -sf http://127.0.0.1:18902/graphql -X POST -H 'content-type: application/json' -d '{"query":"{apiInfo{schemaVersion}}"}' >/dev/null 2>&1 && break
  sleep 0.25
done

GQL() { curl -s "$@" -H 'content-type: application/json'; }

SNAP="$(GQL http://127.0.0.1:18902/graphql -d '{"query":"{player{epoch revision transport{volume playing} eq{preamp bands}} capabilities{playlistEdit}}"}')"
...
ADD="$(GQL http://127.0.0.1:18902/graphql -d "{\"query\":\"mutation{playlistAdd(paths:[\\\"$QTAMP_MUSIC_ROOT/demo.wav\\\"]){ok revision player{playlist{rowCount rows{index text}}}}}\"}")"
contains "$ADD" '"ok":true' "mutation playlistAdd ok"
contains "$ADD" '"rowCount":1' "mutation echo carries refreshed playlist"

GUARD="$(GQL http://127.0.0.1:18902/graphql -d '{"query":"mutation{playRow(row:0,expectPlaylistRevision:999){ok error}}"}')"
contains "$GUARD" '"ok":false' "revision guard rejects as CommandResult"
contains "$GUARD" 'ismatch' "guard error message surfaces"

(sleep 0.6; GQL http://127.0.0.1:18902/graphql -d '{"query":"mutation{play{ok}}"}' >/dev/null) &
