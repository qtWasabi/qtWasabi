...
// qtamp list-only geometry knobs (defined in PleditHostShim.cpp).  Default
// to the classic full-window offsets; the holder renderer sets list-only
// values so draw_pe fills a row-only buffer the Wasabi frame positions.
extern int pe_list_top;       // row-0 y (classic: 22, under the titlebar)
extern int pe_list_reserve;   // chrome height deducted from row count (classic: 60)
extern int pe_list_only;      // 1 → suppress draw_pe's bottom time/status strip
extern int pe_rowheight;      // row pitch (decoupled from the glyph cell size)
...
static void draw_pe_song(HDC hdcout, int pos, wchar_t *name, int time, int sel)
{
	...
	{
		// Row geometry uses the (decoupled) row pitch; the glyph cell stays
		// pe_fontheight via `useFont`.  DT_VCENTER then centres the title in
		// the taller row, reproducing the reference's row leading.
		fontHeight=pe_rowheight;
		right=config_pe_width-20;
		ypos=pe_list_top+pos*fontHeight,
		left=2;   // flush-left like the reference (was 12px indent)
		num_songs=(config_pe_height-pe_list_reserve)/pe_rowheight;
		useFont=font;
	...
	{	
		...
		if (!pos || pos == num_songs-1) 
		{
			...
			if (pos == num_songs-1)
				// Fill the gap below the LAST visible row down to the list
				// bottom.  The classic strip reserves 38px for the status/
				// button bar; in list-only mode the Wasabi frame owns that
				// chrome, so the list bottom is `config_pe_height -
				// pe_list_reserve` (e.g. 95 in a 99px buffer).  Using the
				// hardcoded 38 here made this rect INVERTED whenever the last
				// row sat below y(h-38) — Qt normalises the negative-height
				// rect and fills UPWARD, painting over the bottom rows (only
				// ~4 of 6+ rows survived).
				Rectangle(hdcout, left, ypos+fontHeight, 1+right,
				          config_pe_height-(pe_list_only?pe_list_reserve:38)+1);
		}
...
static void draw_pl2(HWND hwnd, HDC hdc, RECT &r)
{
	if (config_pe_height != 14) 
	{
		{
			int x,t;
			int num_songs=(config_pe_height-pe_list_reserve)/pe_rowheight;
			...
			for (x = 0; x < t; x ++)
			{
				...
				qt=pe_list_top+x*pe_rowheight;
				b=qt+pe_rowheight-1;
				if (IntersectY(qt, b, r.top, r.bottom)
					|| (x == t-1 && config_pe_height-pe_list_reserve >= r.top))
				{
					wchar_t ft[FILETITLE_SIZE] = {0};
		...
		// List-only mode: the Wasabi frame paints the playlist's button bar
		// and total-time readout itself, so draw_pe must not also stamp its
		// classic bottom time/status strip into the row buffer.
		if (!pe_list_only) {
...
// ── qtamp: public render entry (appended to the staged copy) ───────
// draw_pl/draw_pl2 are static; expose a wrapper that renders the whole
// playlist (bg + rows) into a given HDC at an explicit size, bypassing
// the GetClientRect/HWND dependency.  PleditHostShim.cpp calls this.
extern "C" void qtamp_pe_render(HWND hwnd, HDC hdc, int w, int h) {
    // Every draw_pe entry early-returns unless pe_init is set; the real
    // Winamp arms it in draw_pe_init() (the skin-bitmap load is a no-op
    // stub here, so this is safe on the GDI text path).  Idempotent.
    if (!pe_init)
        draw_pe_init();
